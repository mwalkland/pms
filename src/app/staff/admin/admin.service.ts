...
export class AdminService {
  ...
  getAllStaff(): Observable<Staff[]> {
    if (!this.staff) {
      return this.http.get('/admin/getAllStaff')
        .map(response => {
  ...
  modifyProjectSupervisor(staffId: string, studentId: string): Observable<Object> {
    ...
    return this.http.patch('/admin/modifyProjectSupervisor', body, { headers: headers });
  }

  sendReminder() {
    const body = {};
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.post('/admin/sendReminder', body, { headers: headers });
  }
  ...
  updateSuggestedAreas(areas: string[]) {
    ...
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.put('/admin/updateAreasList', body, { headers: headers });
  }

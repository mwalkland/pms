...
export class StudentService {
  ...
  getStaff(): Observable<User[]> {
    if (!this.staff) {
      return this.http.get('/user/getStaff')
  ...
  getAreas(): Observable<string[]> {
    if (!this.areas) {
      return this.http.get('/project/getAreas')
        .map((response: Response) => {
  ...
  getStaffProjects(): Observable<Project[]> {
    if (!this.projects) {
      return this.http.get('/project/getAllStaffProjects')
        .map((response: Response) => {
  ...
  getStudentProject(): Observable<Project> {
    return this.http.get('/project/getStudentProject')
      .map((response: Response) => {
  ...
  addStudentProject(project: Project) {
    ...
    return this.http.patch('/project/addStudentProject', body, { headers: headers })
  }

  createStudentProject(project: Project) {
    const body = JSON.stringify(project);
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.post('/project/createStudentProject', body, { headers: headers })
  }
